...
def GenerateCardFromArticleInformation(articleInformation):
    ...
    image_url = articleInformation.imageUrl
    if(image_url.startswith("https:")):
        test_url = articleInformation.imageUrl + "?width=350&height=275"
        test_url = test_url[8:]
        test_url = "https://rsz.io/" + test_url
        if (str(requests.get(test_url)) != "<Response [500]>"):
            image_url = test_url
    elif(image_url.startswith("http:")):
        test_url = articleInformation.imageUrl + "?width=350&height=275"
        test_url = test_url[7:]
        test_url = "http://rsz.io/" + urllib.parse.unquote(test_url)
        if (str(requests.get(test_url)) != "<Response [500]>"):
            image_url = test_url
